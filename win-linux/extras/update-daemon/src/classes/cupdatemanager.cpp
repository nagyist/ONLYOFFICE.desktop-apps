    ...
    m_socket->onMessageReceived([=](void *data, size_t) {
        ...
        if (params.size() == 4) {
            ...
            case MSG_LoadUpdates: {
                ...
                if (m_pDownloader)
                    m_pDownloader->downloadFile(params[1], generateTmpFileName(TEXT(".zip")));
...
#else
            if (fnmatch("*.json", filePath.c_str(), 0) == 0 || fnmatch("*.zip", filePath.c_str(), 0) == 0) {
#endif
...
# endif
    ...
    // Wait until editors_helper.exe closes
    {
        ...
        if (NS_File::isProcessRunning(app)) {
            NS_Logger::WriteLog(TEXT("Update cancelled. The editors_helper.exe is not closed!"), true);
            return;
        }
    }
