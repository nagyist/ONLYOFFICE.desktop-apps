...
//        _filter_ = joinFilters();
        QString advanced_formats = strcmp(APP_TITLE, "ONLYOFFICE") == 0 ? " *.docxf *.oform" : "";
        _filter_ =  tr("Text documents") + " (*.docx *.doc *.odt *.ott *.rtf *.docm *.dotx *.dotm *.fb2 *.fodt *.wps *.wpt *.xml *.pdf *.djv *.djvu" +
                    advanced_formats + ");;" +
